    ...
    <div className="dashboard-view">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <MetricCard 
          title="Page Views" 
          loading={loading}
          value={loading ? 0 : <CountUp end={stats.pageViews} />} 
        ...
        <MetricCard 
          title="Avg. Session" 
          loading={loading}
          value={loading ? "..." : stats.avgSession}
        ...
        <MetricCard 
          title="Bounce Rate" 
          loading={loading}
          value={loading ? 0 : <CountUp end={stats.bounceRate} suffix="%" />} 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 shadow-lg">
          <h3 className="font-bold mb-6">Traffic by Device</h3>
          <div className="space-y-4">
            {loading ? (
               <div className="h-32 bg-gray-700/20 rounded animate-pulse"></div>
            ) : (
              <>
        ...
        {/* Top Pages Table */}
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 shadow-lg min-h-[300px]">
          <h3 className="font-bold mb-4">Top Pages Visited</h3>
          <table className="w-full text-sm text-left text-gray-400">
            <thead className="text-xs uppercase bg-gray-700/50 text-gray-200">
              <tr>
                <th className="px-3 py-2 rounded-l-lg">Page URL</th>
                <th className="px-3 py-2 rounded-r-lg text-right">Views</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-700">
              {loading ? (
                 <tr><td colSpan="2" className="p-8 text-center">Loading data...</td></tr>
              ) : (
                pages.map((page, idx) => (
                  <tr key={idx} className="hover:bg-gray-700/30 transition-colors animate-in slide-in-from-right-4 duration-500">
                    <td className="px-3 py-3 font-mono text-blue-300">{page.path}</td>
                    <td className="px-3 py-3 text-right font-medium text-white">
                        <CountUp end={page.views} />
                    </td>
                  </tr>
                ))
              )}
...
const MetricCard = ({ title, value, icon, color, loading }) => (
  <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 flex items-center justify-between shadow-lg hover:translate-y-[-5px] transition-transform duration-300">
    <div>
      <p className="text-gray-400 text-sm">{title}</p>
      {loading ? (
        <div className="h-8 w-24 bg-gray-600 rounded animate-pulse mt-2"></div>
      ) : (
