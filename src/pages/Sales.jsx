...
import { exportToCSV } from '../utils/exportHelper'; // Import export helper
...
const Sales = () => {
  ...
  // 2. EXPORT HANDLER
  const handleExport = () => {
    exportToCSV(filteredTransactions, 'sales_transactions');
  };
      ...
      <div className="page-title-area">
        ...
        <div className="flex gap-2">
            <button 
              ...
              className={`secondary-btn flex items-center gap-2 ${showOnlyCompleted ? 'bg-blue-500/20 text-blue-300 border-blue-500/50' : ''}`}
            >
                <Filter size={16} /> {showOnlyCompleted ? 'Show All' : 'Filter Completed'}
            </button>
            <button onClick={handleExport} className="primary-btn flex items-center gap-2">
                <Download size={16} /> Export CSV
            </button>
        </div>
      </div>

      {/* Financial Overview Cards (Same as before) */}
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden shadow-lg min-h-[400px]">
        <div className="p-6 border-b border-gray-700 flex justify-between items-center">
            ...
            <input 
              ...
              className="search-input py-1 px-3 text-sm w-48"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
        </div>
        <table className="w-full text-left text-sm text-gray-400">
          <thead className="bg-gray-700/50 text-gray-200 border-b border-gray-700">
            <tr>
          ...
          <tbody className="divide-y divide-gray-700">
            {loading ? (
              ...
              filteredTransactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-gray-700/30 transition-colors animate-in slide-in-from-bottom-2 duration-500">
                  <td className="p-4 font-mono text-blue-400">#INV-{tx.id}</td>
                  <td className="p-4 text-white font-medium">{tx.user}</td>
                  <td className="p-4">{tx.date}</td>
                  <td className="p-4 text-white font-bold">
                    <CountUp end={tx.amount} prefix="$" suffix={Number.isInteger(tx.amount) ? ".00" : ""} />
                  </td>
                  <td className="p-4">
...
const StatCard = ({ loading, label, value, prefix, suffix, highlight }) => (
  <div className={`rounded-xl p-6 border shadow-lg hover:-translate-y-1 transition-transform duration-300
    ${highlight ? 'bg-blue-600 border-transparent text-white shadow-blue-900/20' : 'bg-gray-800 border-gray-700 text-white'}`}>
      <p className={`text-sm font-medium ${highlight ? 'text-blue-100' : 'text-gray-400'}`}>{label}</p>
      {loading ? (
        <div className={`h-8 w-24 rounded animate-pulse mt-2 ${highlight ? 'bg-blue-400/50' : 'bg-gray-600'}`}></div>
      ) : (
